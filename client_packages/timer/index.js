...
mp.timer = {
                ...
                try {
                    if (timers[i].time <= Date.now()) {
                        ...
                        if (timers[i].interval != null) {
                            ...
                            if (logId) {
                                mp.log(`Timer with id ${logId} removed before work`);
                            }
                            timers.splice(i, 1);
                            i--;
                        }
                        if (logId) {
                            mp.log(`Timer with id ${logId} start`);
                        }
                        handler();
                        if (logId) {
                            mp.log(`Timer with id ${logId} done`);
                        }
                    }
                } catch (error) {
                    timers.splice(i, 1);
                    i--;
                    mp.log(`Client timer error: ${JSON.stringify(error)}`);
                }
            }
        }, duration);
    ...
    add(handler, time, isInterval = false, isLog = false) {
        ...
        if (time === 0) {
            if (isLog) mp.log(`Timer with timeout = 0 done`);
            handler();
            return;
        }
        let id = gId++;
        if (isLog) {
            mp.log(`Add timer ${JSON.stringify({
                id: id,
                handler: handler,
                time: Date.now() + time,
    ...
    remove(timer) {
        ...
        if (index !== -1) {
            if (timers[index].isLog) {
                mp.log(`Remove timer with id ${timers[index].id}`);
            }
